...
class UsersViewModel(
    ...
    private fun initNetworkState() {
        viewModelScope.launch {
            userRepository.networkState.observeForever {
    ...
    suspend fun insertBookMarkUsers(users:Users) {
        userRepository.insertBookMarkUsersFromUsers(users)
    }

    // 북마크 체크 해제 시 BookMarkUsers에 해당 Users 데이터 삭제
    suspend fun deleteBookMarkUsers(users:Users) {
        userRepository.deleteBookMarkUsersFromUsers(users)
    }
    ...
    fun reTryListener(connected:Boolean) {
        viewModelScope.launch(Dispatchers.IO) {
            userRepository.reTryListener(connected)
        }
    }
}